...
def calculate_m5_sl_tp(
    ...
    if direction == 1:  # BUY
        ...
        # Micro C1 Target check: use C1 if net headroom after front_pad >= 1.20x SL
        if c1 and c1 > entry_price:
            raw_c1_dist = c1 - entry_price
            if ((c1 - front_pad) - entry_price) >= (1.20 * sl_dist) and raw_c1_dist <= (max_tp_dist * 1.15):
        ...
        sl = entry_price + sl_dist
        # Micro F1 Target check: use F1 if net headroom after front_pad >= 1.20x SL
        if f1 and f1 < entry_price:
            raw_f1_dist = entry_price - f1
            if (entry_price - (f1 + front_pad)) >= (1.20 * sl_dist) and raw_f1_dist <= (max_tp_dist * 1.15):
...
class MarketScannerM5(MarketScanner):
    ...
    def scan_fast_radar(self, mt5_connector=None) -> List[CandidateSetup]:
        ...
        for cand in candidates:
            ...
            # Strict R:R Gate: Veto any candidate with realized R:R under 1.20:1
            if geom.get("risk_reward", 0.0) < 1.20:
                logger.info(f"[M5 SCANNER VETO] {sym} REJECTED: R:R {geom.get('risk_reward', 0.0):.2f}:1 is below 1.20:1")
                continue
            ...
            cand.metadata["has_runner"] = False
            cand.metadata["is_reanchored_limit"] = False
