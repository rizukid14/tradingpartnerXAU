...
6. **RSI DIRECTION + DIVERGENCE FACTS** (`_compute_rsi_analysis`, 13 Agustus): RSI sekarang vs 5 candle lalu (RISING/FALLING/FLAT, threshold ±1.0) + fakta divergence **tanpa label** ("price made LOWER LOW 4385.00 vs 4390.00 while RSI made HIGHER LOW 44.0 vs 40.0 - momentum diverging from price") — label "DIVERGENCE" dari bot dihindari karena rawan false positive (local-low sederhana di window 20) dan LLM cenderung anchor ke label (kasus EURAUD di A/B test).

**Yang DI-SKIP (sengaja):** #3 Candle Character (LLM cukup pinter baca 3 candle terakhir dari data mentah + label threshold heuristik berisiko mislabel), #6 Multi-Scenario & #11 Counter-Argument (butuh perubahan schema output + parser, nyusul), #8 Session folklore (pseudo-fact), #12 Zone playbook (inject bias — kontradiksi ANALYSIS FREEDOM), #10 Confidence Calibration (sample size < 50 per direction).
