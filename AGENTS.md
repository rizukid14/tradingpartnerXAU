...
- **TRADING_MODE** (config/.env/UI dashboard): `"xau"` (default, XAU only) | `"xau_pairs"` (**parallel scan**: SEMUA simbol di-scan pada SETIAP candle M5 — 3× LLM call per candle, 1 call per pair; dulu 5×, dikurangi 11 Agustus karena kemahalan). Pool = `WEEKDAY_SYMBOL` + `FX_PAIR_SYMBOLS` (default `EURJPY-ECN, GBPCHF-ECN` — semua cross non-USD, korelasi rendah dengan XAU & tidak saling berkorelasi EUR/JPY; spread hasil scan: EURJPY ~0-1, GBPCHF 0, tick value GBPCHF ~2× EURJPY), dipotong `MAX_ROTATION_SYMBOLS` (3). Weekend: FX tutup → pool jatuh ke XAU (atau BTC kalau `ENABLE_BTC_ROTATION`). Max posisi & daily loss **aggregate semua simbol** (bukan per-simbol). Implementasi: `run_trading_cycle()` → loop `for sym in pool: config.SYMBOL = sym; _run_cycle_for_current_symbol()` (post-mortem 1× aggregate per candle, macro/MTF per-symbol di-populate di dalam loop).
...
30. **Default SL/TP per-symbol + multiplier ATR per AI mode** (11 Agustus, sesi ini):
    - `default_sl_points_for()`/`default_tp_points_for()` sekarang per-symbol: **XAU 400/800** (dinaikkan dari 300/600 karena ATR M5 XAU ~302 pts → gate butuh SL ≥ 1.25×ATR ~378; default 300 malah gagal lolos gate sendiri), **EURJPY 50/100** (5/10 pips), **GBPCHF 40/80** (4/8 pips), BTC 50000/100000. Sebelumnya semua non-crypto jatuh ke 300/600 yang untuk FX = 30 pips = 12-17× ATR M5 (kepanjangan, lot jadi kecil, TP gak pernah kena).
