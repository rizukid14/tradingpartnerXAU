...
| `src/analytics/basket_sync_engine.py` | **CBSS Engine (9/10 Sep 2026)** — Pemetaan konstituen 8 keranjang mata uang, kalkulasi bilateral runway ZCE per-pair ($C_1/F_1$ vs $C_2/F_2$), Local Pair G3 Wall Veto (The EURAUD Law), Basket Concurrency Cap (max 2 posisi per keranjang searah / auto-route ke Paper Trade `SKIPPED_CBSS_BASKET_CAP`), **BSSI (Basket Structural Saturation Index $\ge 70\%$)**, dan **Bilateral Composite Champion Selector (`select_basket_champion`)**. |
