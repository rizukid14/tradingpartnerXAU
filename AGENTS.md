...
- **`TRADING_MODE = "scanner"` (Default)**: Universe **26 simbol FX Terkurasi** dipindai paralel tiap 60 detik oleh **Stage 1 Fast Radar** (`market_scanner.py`) — mekanisme M1 (Universal Liquidity Sweep & SFP), M2 (Trend-Aligned Pullback), M3 (Multi-Touch Breakout Retest), M4 (DBD / RBR Breakout Continuation) — dengan timeframe struktural **H1 untuk seluruh 26 FX pair (termasuk JPY Crosses pasca unifikasi 4 Sep 2026)**. Hanya **8–15 setup A+ per hari** yang lolos ke **Stage 2 (3-LLM Consensus Jury)**. Hemat ~85% token API vs full-cycle scan.
- **BTCUSD.c (Bitcoin)**: Rotasi 24/7 aktif khusus di **Virtual Paper Trade (`shadow_tracker`)** via `ENABLE_BTC_247_PAPER=True` (0 token API, 0 risiko modal MT5 live). Jika paper-only dinonaktifkan di akhir pekan, max 2 posisi, risk 0.50%.
- **Pure Quant Direct Execution (`quant-trade-noAI`)**: Mode `ENABLE_LLM_JURY=False` mengeksekusi setup quant langsung ke MT5 (0 token API) dengan institutional safety check (`MT5_ACCOUNT_MODE=live`).
- **XAUUSD-ECNc (Gold)**: Aktif kembali pada weekday khusus di **Virtual Paper Trade (`shadow_tracker`)** via `ENABLE_XAU_PAPER=True` (0 token API, 0 risiko modal MT5 live). Dikarantina 100% dari eksekusi riil akun Cent `VTMarkets-Live 3`, namun aktif dipindai radar dan ditampilkan di Dashboard Cockpit 8-Gate X-Ray.
...
- **Akun**: **LIVE Cent** `VTMarkets-Live 3` (login `27556325`), magic `20260625`, Waktu **WIB** (Asia/Jakarta) (Demo fallback: `1157958` `VTMarkets-Demo`).
...
| `src/indicators/lux_smc_official.pine` | **Official LuxAlgo SMC v6 Source Code (49.8 KB)** — Sumber rujukan resmi Pine Script v6 untuk SMC, BOS/CHoCH, Order Blocks, FVG, Equal Highs/Lows, dan Dealing Range OTE |
| `src/analytics/pattern_engine.py` | **Macro Dynamic Envelope & SMC Dealing Range Engine** — Decoupled Dual-Track, Active Structural Impulse Leg Anchor (menggantikan `np.argmax(highs)` naif), dan Sequential SMC Structural Hierarchy Tracking (anti-false `HH` pada retracement) |
...
| `src/analytics/basket_sync_engine.py` | **CBSS Engine & Lead-Lag Relay (9/10/11 Sep 2026)** — Pemetaan konstituen 8 keranjang mata uang, kalkulasi bilateral runway ZCE per-pair ($C_1/F_1$ vs $C_2/F_2$), Local Pair G3 Wall Veto (The EURAUD Law), Basket Concurrency Cap (max 2 posisi per keranjang searah / auto-route ke Paper Trade `SKIPPED_CBSS_BASKET_CAP`), **Eksklusif G3 Wall Exhaustion (0.35x ATR, G1/G2 Penetrable)**, **Bilateral Composite Champion Selector (`select_basket_champion`)**, dan **Lead-Lag Liquidity Relay Engine (`filter_and_rank_batch_candidates`)**. |
| `src/analytics/zone_confluence_engine.py` | **ZCE (True Zonal Bands, Dynamic EMA Bands 20/50/100/200 & Special G3 Protocol)** — peta zona 6-TF × multi-horizon, True Zonal Bands wick-to-body `[0.05x, 0.35x ATR_TF]`, Dynamic EMA Bands (20/50/100/200 di H1/H4/D1, bobot 0.25), Special G3 Protocol (skor $\ge 8.5$ + wajib confirmed macro swing/psych anchor; micro-clutter di-cap G2), Pure Sequential 4-Station Natural Ladder (C1..C4, F1..F4), hook `zce_walls` → MSE (`ZCE_ENABLED=true`, `ZCE_MODE=full`) |
...
| `src/analytics/position_manager.py` | 2-Stage Trailing (H1 Breathing 65-90% TP, M30 Terminal Lock $\ge$90% TP), BEP 45-55%, partial close 50%, time-decay stagnation, pre-rollover shield, **Midday Retracement Guard (The 65% Pullback Boundary Rule, 11:00–13:00 WIB)**, dan **Pre-News Emergency Shield (18:45 WIB 30-Minute Tier-1 Stand-Off Shield)** |
| `src/analytics/macro_strategic_engine.py` | **Barrier Chamber State Machine** (6-TF Native `MN1/W1/D1/H4/H1/M30`), **Dual-Horizon Lookback Engine W1 (Secular 156b vs Intermediate 52b, Anchor Peak Law & Upper Tangent Slope Envelope)**, Density Cluster Scoring ($C_1, C_2, F_1, F_2$), Interaction Sequence Tracking (`['F1_SWEEP', 'C1_SWEEP']`), 7-State Engine, Pair-Calibrated SL Floor (35p Crosses), dan **Diurnal Confluence Timing (`evaluate_session_confluence_timing`)** (Tokyo/London/NY phase mapping & `GRADE_B_C1` vs `GRADE_A_PLUS_C2` target directives) |
| `src/core/cli_theme.py` | UI Theme & ASCII Bento Box HUD: Palet kontras institusional (Floor Sky Blue `UI.CYAN` / Ceiling Warm Amber `UI.YELLOW`), Radar mechanism badges M1..M4, adaptif Pure Quant No-LLM vs 3-AI Jury, dan dynamic status clock line format `[POOL 26 PAIRS (H1) \| HH:MM:SS]` |
| `dashboard.py` | Multi-Pair Cockpit Dashboard & 8-Gate X-Ray Surveillance (Pure Sequential 4-Station Natural ZCE Ladder F1..F4 / C1..C4, Expanded 300-Bar H1 Native Viewport, Pip-Aware Zone Consolidation $\ge 15\text{p}$, bilateral C1/F1 projection, live candles, telemetry radar M1..M4, dedicated Economic News Blackout Shield Gate) |
| `dashboard_assets.py` | Single-Page Application (SPA) Assets: Lightweight Charts, Timeframe Selector (H1/M30/M5), Palet Kontras Institusional (Floor Sky Blue `#38bdf8` / Ceiling Warm Amber `#fbbf24`, 1.2px hairline G3), Gradasi Opacity C1..C8 / F1..F8 (C8 = 50%), Radar Institutional Contrast Palette (M1 `#fb923c`, M1B `#ec4899`, M2 `#818cf8`, M3 `#c084fc`, M4 `#34d399`), Margin Kiri 100% Khusus ZCE, Spasial Grid 3-Kolom Watchlist (C1 over F1, Trig entry distance) & Standardisasi Non-Emoticon (0% Emoji) |
...
6. **Risk-based lot sizing**: lot = `(equity × risk%) / (SL_pts × usd_per_point) × session_mult`. FX 1.0%, BTC 1.5%. Multiplier sesi: Asia 1.2x, London 1.0x, NY 0.50x flat.
...
- **LuxAlgo MCP Server Integration & Official Source (`https://mcp.luxalgo.com/mcp`) (12 Sep 2026)**:
  - **Server-Sent Events (SSE) Streamable HTTP MCP Server**: Terintegrasi permanen di `~/.gemini/config/mcp_config.json` dengan identitas `"luxalgo"`. Free, keyless, dan read-only, menyediakan 42 tools instan untuk riset kuantitatif.
  - **Katalog Ensiklopedia 800+ Indikator**: Akses langsung ke konsep dan algoritma teknikal via `library_search`, `library_get_concept`, `library_get_indicator`, dan `library_get_source_code`.
  - **Official LuxAlgo SMC v6 Source Code**: Seluruh source code resmi Pine Script v6 dari LuxAlgo Smart Money Concepts (49.8 KB, 848 baris) disimpan permanen di [`src/indicators/lux_smc_official.pine`](file:///c:/Data%20%28D%29/Vibecoding/tradingpartnerXAU/src/indicators/lux_smc_official.pine) sebagai sumber acuan tunggal formula BOS, CHoCH, Order Block, FVG, dan Dealing Range.
  - **Uji Kuantitatif Institusional**: Dilengkapi tools `edge_report` (evaluasi $P(\text{outcome} \mid \text{conditions})$ dengan Wilson 95% Confidence Interval) dan `propfirms_simulate_trades` (Monte Carlo block bootstrap simulator).
- **Rekonstruksi Dealing Range SMC Sejati & Hierarki Struktur (12 Sep 2026 — Resolusi Anomali Puncak Retracement)**:
  - **Active Structural Impulse Leg Anchor**: Menggantikan logika naif `np.argmax(highs)` / `np.argmin(lows)` 120-bar dengan deteksi origin impulse leg aktif (ComLucro SMC Standard). `RANGE HIGH (BSL)` dikunci pada Origin Swing High (`LH`) yang memicu breakdown, dan `RANGE LOW (SSL)` dikunci pada `LL` dasar. Mencegah garis dealing range nyangkut di pucuk purba masa lalu.
  - **Sequential Structural Hierarchy Tracking**: Melarang keras penandaan false `HH` pada pantulan retracement di dalam downtrend. Swing high baru hanya berhak dilabeli `HH` jika berhasil menembus Active Structural High sebelumnya dengan *noise buffer* $0.15\times\text{ATR}$ (~1.6 pips). Seluruh pantulan internal di dalam struktur bearish secara otomatis dilabeli `LH` (merah).
...
- **Resolusi Multi-Mekanisme Radar & Confluence Fusion (11 Sep 2026 - Malam II)**:
  - **Peniadaan `continue` Prematur**: Seluruh 5 mekanisme (M1, M1B, M2, M3, M4) dipindai tuntas pada setiap simbol tanpa short-circuit, ditampung ke `sym_candidates`.
  - **Confluence Fusion**: Setup searah dalam jarak $\le 0.35\times\text{ATR}$ dilebur menjadi tiket tunggal konfluensi berkekuatan ganda (misal `M1+M3_CONFLUENCE`, `M2+M3_CONFLUENCE`), dengan upgrade tier/grade (`GRADE_A` $\rightarrow$ `GRADE_A+` $\rightarrow$ `GRADE_S`).
  - **Resolusi Setup Berlawanan**: Seleksi pemenang berdasarkan skor multi-dimensi: Makro MSE ($2.0\times$), Mikro CSM Delta ($1.5\times$), Panjang Runway ZCE ($1.5\times \min(3.5, \text{runway\_atr})$), serta penalti/bonus dinding kamar ZCE.
  - **Sinkronisasi 1:1 Dashboard Cockpit**: `_elect_primary_standby` di `dashboard.py` diselaraskan 1:1 dengan engine radar: deteksi Dealing Range ekstrem ($dr\_pos \ge 0.80$ fade ceiling / $\le 0.20$ fade floor), kapasitas runway sehat ($\ge 0.25\times\text{ATR}$), dan sorting hirarki institusional (`Confluence > Preferred Direction > Active > Actionable > Healthy Runway > Proximity`).
- **Penguncian Dealing Range Absolut Radar M1, M2 & M3 (11 Sep 2026 — Resolusi Anomali EURGBP)**:
  - **M1 Sweep Zone Integrity**: M1 Sweep SELL wajib berada di area Premium ($dr\_pos \ge 0.55$ atau $intraday\_dr\_pos \ge 0.50$); M1 Sweep BUY wajib berada di area Discount ($dr\_pos \le 0.45$ atau $intraday\_dr\_pos \le 0.50$). Bypass naif `ref_top/ref_bot` dihapus total. Plafon kandidat M1 SELL dilarang di discount ($<0.50$), lantai kandidat M1 BUY dilarang di premium ($>0.50$).
  - **M2 Pullback Boundary Guard**: Veto BUY jika $dr\_pos \ge 0.80$ dan jarak ke $C_1 < 0.40\times\text{ATR}$ (`[PULLBACK BUY CEILING COLLISION]`); Veto SELL jika $dr\_pos \le 0.20$ dan jarak ke $F_1 < 0.40\times\text{ATR}$ (`[PULLBACK SELL FLOOR COLLISION]`).
  - **M3 Exhaustion Retest Guard**: Veto BUY jika $dr\_pos \ge 0.85$ tanpa physical breach $C_1$ (`[BREAKOUT BUY EXHAUSTION]`); Veto SELL jika $dr\_pos \le 0.15$ tanpa physical breach $F_1$ (`[BREAKOUT SELL EXHAUSTION]`).
- **Rekonstruksi Geometri Trade Kuantitatif & Proteksi Modal (11 Sep 2026)**:
  - **Plafon Lot Maksimal Akun Cent**: `MAX_POSITION_LOT = 0.50` di `.env` dan `config.py`. Mengunci lot efektif maksimal $\le 0.50$ lot pada akun Cent, mengeliminasi lonjakan lot abnormal saat SL ketat.
  - **Floor Turunan Friksi (`FRICTION_FLOOR_DIVISOR = 0.20`)**: Menjamin total friksi (spread + komisi round-turn) tidak melebihi 20% dari jarak Stop Loss (`config.friction_floor_points(spread_pts)`).
  - **Formula SL Seimbang Tiga Suku di Atlas DNA**: $\text{SL} = \max(\text{struct\_dist} + \text{buffer}, 1.00\times\text{ATR}, \text{fric\_floor})$ dengan $\text{buffer} = \max(0.15\times\text{ATR}, (2\times\text{spread} + 10)\times\text{pt})$. Mencegah formula degenerate bernilai nol saat limit entry berimpit dengan anchor M2/M3.
  - **Dekopel TP dari Kelipatan SL (Mode ZCE Produksi)**: Mempertahankan target struktural ZCE alami apa adanya selama memenuhi kapasitas runway Grade B floor ($0.50\times\text{SL} + \text{friksi}$) tanpa menaikkan TP secara artifisial yang merusak win rate.
  - **Defensif London 15:00–17:59 WIB**: Melarang pemasangan `sell_limit` pasif pada jendela pembukaan London yang ber-winrate rendah (`ENABLE_LDN_DEFENSIVE_WINDOW = True`).
  - **Target Jauh Bersyarat $\ge 2.5\times\text{ATR}$**: Target TP $\ge 2.5\times\text{ATR}$ hanya diizinkan jika didukung oleh Tokyo session + $|\text{CSM Delta}| \ge 2.00$ atau konfirmasi dinding breached (`c1_breached` / `f1_breached`).
  - **Proteksi Momentum Lilin & Integritas Kamar (Resolusi Kasus AUDNZD)**:
    * **Anti-Marubozu Waterfall Guard**: Melarang keras order limit menabrak candle Marubozu ekspansif counter-trend (body $\ge 55\%$, rejection wick $< 20\%$) di M3 Retest dan M2.
    * **Penutupan Kebocoran Mid-Chamber MSE**: Menghapus bypass naif `if is_limit_retest: continue`. Setup SELL wajib di area plafon sejati ($entry \ge C_1 - 0.25\times\text{ATR}$) dan BUY di lantai sejati ($entry \le F_1 + 0.25\times\text{ATR}$). Entry di koridor transit dibekukan (`[MID-CHAMBER FREEZE]`).
    * **Projected SBR / Breached Wall Law**: Filter `_is_zce_wall_breached` melarang level yang telah ditembus badan lilin dijadikan anchor limit.
- **Lantai & Plafon SL/TP (`_apply_sltp_rules` di `consensus.py` — realita 8 Sep 2026)**:
  - **Segmented Safety Floors (3 Sep 2026 / 4 Sep Unified H1)**:
    ...
    * **M4 Breakout Continuation (DBD / RBR Micro-Basing)**: Tunduk pada segmented safety floor & Net R:R (`M4_STRUCTURAL_FLOORED`), entry pending limit di batas basing box, SL di balik basing floor/ceiling.
...
- **Dead Zone & Sesi Operasional**: Dead Zone 00:00–07:00 WIB (FX & XAU skip; BTC 24/7); **Night Freeze 23:00–07:00 WIB (pembukaan order FX baru dibekukan, BTC exempt)**; Sesi Tokyo 07:00–14:00 WIB (khusus driver aktif JPY/AUD/NZD, pair Barat locked, lot 1.20x); Sesi London Core 14:00–18:00 WIB (all 26 FX permitted, **lot multiplier 1.00x normal**); **Sesi New York 18:00–00:00 WIB (flat lot multiplier 0.50x untuk semua grade setup, M3 Breakout Retest dialihkan ke Virtual Paper Trade `SKIPPED_NY_M3_PAPER` untuk pengumpulan sampel $N \ge 60$)**.
...
- **CBSS Local Pair G3 Wall Veto, Decoupled Runway & Basket Concurrency Cap (9/10 Sep 2026)**: Ambang runway ZCE dipisahkan: M4 Systemic Flow $\ge 1.20\times\text{ATR}$, setup chamber M2/M3 $\ge 0.60\times\text{ATR}$ (`CBSS_MIN_CHAMBER_RUNWAY_ATR`). Pair ditolak jika menempel benteng G3 lawan ($dist \le 0.35\times\text{ATR}$). Maksimal 2 posisi aktif per keranjang mata uang searah. Jika kuota MT5 penuh, setup A+ otomatis dialihkan ke Virtual Paper Trade (`SKIPPED_CBSS_BASKET_CAP`, 0 token API) agar data peluang teknikal tetap terpantau aktif tanpa idling.
- **HTF W1 Slope Barrier & Dual-Horizon Lookback (11 Sep 2026)**: Proyeksi descending trendline W1 hasil *Outer Tangent Envelope / Anchor Peak Law* difungsikan sebagai penggaris visual presisi tinggi di dashboard & context alert (non-blocking), sementara validasi struktural gating tetap dipegang oleh ZCE Horizontal Zones, SMC Order Blocks, dan Liquidity Sweeps.
...
- **Proteksi Posisi Real-Time (`position_manager.py`)**:
  - **Break-Even (BEP)**: Diselaraskan ke **60% TP** (`BREAK_EVEN_TRIGGER_TP_PCT = 0.60`, padding komisi round-trip + Pocket Profit 15 pts / 1.5 pips); M4 di **70% TP**. Menghilangkan premature BEP di retracement wick wajar. Sinergi 1:1 antara `position_manager.py` dan `shadow_tracker.py`.
  - **Partial Close (TP1)**: **DINONAKTIFKAN TOTAL (`PARTIAL_CLOSE_ENABLED = False`)** demi mempertahankan 100% volume trade hingga mencapai TP penuh (mencegah pemotongan asimetris rasio Win/Loss).
  - **3-Tier Progressive Trailing Ladder (11 Sep 2026)**:
    * **Tier 1 (Trigger $\ge$ 75% TP)**: Kunci floating profit sebesar **50% TP**.
    * **Tier 2 (Trigger $\ge$ 90% TP)**: Kunci floating profit sebesar **80% TP**.
    * **Tier 3 (Terminal Lock $\ge$ 95% TP)**: Kunci floating profit sebesar **90% TP** sebelum sentuhan target distal.
  ...
  - **Pending Order Harmonisasi Invalidation CSM & Macro Alignment**: pembatalan pending limit order diselaraskan dengan scanner threshold ($|\text{csm\_delta}| \ge 1.0$, `PENDING_CSM_OPPOSED_THRESHOLD`), dengan proteksi Macro Alignment (order tidak dibatalkan oleh CSM opposed moderat jika didukung bias makro $\ge 0.35$ BUY / $\le -0.35$ SELL). Aktif via `ENABLE_PENDING_CSM_CANCEL=true`.
...
2. **4 Mekanisme Eksekusi Stage 1 Radar**:
   ...
   - **M3: Multi-Touch Breakout Retest + M5 Micro-Rejection Filter (H1/M30)** — break level struktural + konfirmasi M5 rejection wick $\ge 25\%$ pada retest (mengeliminasi 75.7% waterfall penetration) + **Pre-Breakout/Breakdown Context Validator (10 Sep 2026)**: verifikasi batas riil $\ge 3-5$ bar sebelum break guna mengeliminasi 100% false SBR/RBS (mencegah salah klasifikasi Resistance/Support Liquidity Sweep SFP sebagai breakout kelanjutan).
   - **M4: DBD / RBR Breakout Continuation (H1 Pure Technical Basing — 10 Sep 2026)** — Dimurnikan menjadi pola aksi harga teknikal lokal (Breakout Impulsif $\ge 50\%$ body ratio menembus swing 20 bar $\rightarrow$ High-Tight / Low-Tight Basing $2-6$ bar kompresi $\le 0.35\times\text{ATR}$ $\rightarrow$ Buy Limit di atap basing / Sell Limit di lantai basing). Sepenuhnya independen dari rolling 720-bar currency z-score. Systemic Flow tetap hidup mandiri sebagai Gate Makro Layer 0.
   - **CSM Telemetry Decoupling (10 Sep 2026)** — CSM Hard Veto & Dynamic Bailout dinonaktifkan (`ENABLE_CSM_FLOW_FILTER=false` & `ENABLE_CSM_DYNAMIC_BAILOUT=false`), beralih ke Telemetri / Observasi murni (Gate 4/5 `OBSERVE` Cyan) pasca bukti empiris trade CSM Opposed mencetak WinRate 66.7% (+$130) vs Aligned merugi -$914.
...
80. **ZCE Strict Physical Partitioning, Eliminasi Anomali Inversi & Penyelarasan Native H1 (300 Bar Expanded)** (10 September 2026 — Pagi):
    - **Strict Physical Partitioning**: Zona di bawah harga live hanya sah masuk `floor_cands` (RBS jika tembus $\ge probe\_tol$); zona di atas harga hanya sah masuk `ceil_cands` (SBR jika tembus $\ge probe\_tol$). Presisi pembulatan float `digits` mengeliminasi 100% anomali inversi fisik pada seluruh 26 pair universe di MT5.
    - **Ekspansi History H1 ke 300 Bar**: Memperluas candlestick H1 dari 150 bar ke 300 bar (~2.5 minggu perdagangan) dan mengeliminasi tombol H4 pada chart agar 100% sinkron dengan timeframe eksekusi bot.
    - **Verifikasi Kuantitatif**: 242/242 unit tests 100% PASS dalam 38.42 detik; 0 anomali inversi chamber pada MT5 live feed.
81. **Restorasi Proven ZCE Baseline, Hierarchical Confluence Melting (Pip-Aware Spacing) & Pure Sequential 4-Station Natural Ladder** (10 September 2026 — Pagi II):
    - **Eliminasi Micro-Noise Swings**: Menghapus injeksi uncurated micro-swings M30/H1 yang membanjiri chart dengan 39 klaster sempit; mempertahankan primitif proven inti (OB, FVG, EQH/EQL, FRVP POC/VAH/VAL, dan Atlas DNA Psychological stations 0000 & 50).
    - **Peleburan Spasial Pip-Aware ($\ge 15\text{ pips}$)**: Mengunci `tol` dan `min_sep` dengan batas minimum 15 pips (`max(0.50 * atr_h1, 15.0 * pip_val)`), menyatukan level berdekatan menjadi satu benteng zonal band yang utuh dan lega.
    - **Pure Sequential 4-Station Progression**: Menghapus pembajakan slot G3 ekstrim di F4/C4; seluruh tangga bergerak sekuensial murni keluar dari harga live secara kontigu (mengeliminasi false jump 400 pips ke level multi-tahun).
    - **Verifikasi Kuantitatif**: 242/242 unit tests 100% PASS dalam 26.34 detik; 0 anomali fisik dan tangga sekuensial teratur di seluruh 26 pairs scanner.
82. **ZCE Self-Adaptive ATR-Aware Separation, Macro Fortress Supremacy & Low-Beta Pair Wall Restoration** (10 September 2026 — Pagi III):
    - **Akar Masalah**: Batas kaku floor 15 pips (`min_sep = max(0.50 * atr_h1, 15.0 * pip_val)`) menyebabkan 17 dari 26 simbol FX mendrop benteng makro terkuatnya (`GRADE_3_MACRO`) karena pada pair low-beta (AUDCHF ATR 6.5p, EURGBP ATR 3.9p, EURUSD ATR 8.4p), 15 pips setara dengan 1.8x s/d 3.8x ATR.
    - **Skalasi Dinamis Adaptif**: Mengganti batas kaku dengan formula adaptif ATR: `pip_sep = min(15.0 * pip_val, 0.75 * atr_h1)`, `min_sep = max(0.35 * atr_h1, pip_sep)`, `min_ch = max(0.50 * atr_h1, pip_sep)`, dan `tol_pip = min(8.0 * pip_val, 0.40 * atr_h1)`.
    - **Macro Supremacy & Spatial Conflict Resolution**: Mengeliminasi kelemahan greedy selection di mana kandidat minor G1 mendrop benteng G3 makro di dekatnya. Level dengan grade lebih tinggi (`GRADE_3_MACRO` rank 3) atau skor lebih tinggi secara otomatis memenangkan slot stasiun.
    - **Penyelarasan Dashboard (`dashboard.py`)**: `_consolidate_zce_zones` dan visual chart ladder diselaraskan dengan formula adaptif yang sama, memulihkan benteng makro di seluruh visualisasi GUI.
    - **Eliminasi Pseudo-Floor Midpoint (`dashboard.py`)**: Menyelaraskan klasifikasi cluster mentah di `_consolidate_zce_zones` dengan aturan fisik ZCE. Mengeliminasi bug di mana midpoint resistance (`1.35569` GBPUSD) dibajak menjadi F1 saat harga live menusuk di dalam zona resistance. F1 GBPUSD kembali bersih ke support sejati `1.35368` dan C1 ke `1.35736`.
    - **Verifikasi Kuantitatif Penuh**: Seluruh 242 unit tests 100% PASS; AUDCHF memulihkan C2 0.58567 (G3 Macro, Skor 15.49) dan F3 0.58241 (G3 Macro, Skor 10.91), EURUSD memulihkan C1 1.16412 (G3 Macro, Skor 11.38).
83. **CBSS Basket Saturation Direct-to-Paper Trade Routing (`SKIPPED_CBSS_BASKET_CAP`)** (10 September 2026 — Siang):
    - **Penyelamatan Peluang A+**: Saat kuota keranjang MT5 penuh (2 posisi aktif per arah), setup A+ pada pair ke-3/ke-4 tidak dibuang fatal melainkan dialihkan langsung ke Paper Trade (`shadow_tracker`) dengan tag `SKIPPED_CBSS_BASKET_CAP` (0 token API, 0 risiko modal MT5, 100% data tracking aktif).
84. **Confluence Timing, Basket Saturation (BSSI), Midday Retracement Guard (65% Rule) & Pre-News Shield** (10 September 2026 — Siang/Sore):
    - **Temuan Empiris 130.000 Candle H1**: 80.8% dorongan pagi mengalami retracement $\ge 40\%$ di jam 11:00–13:00 WIB. Retracement normal $\le 65\%$ memiliki probabilitas **81.6% melanjutkan tren di London**; retracement $> 65\%$ gagal **53.4%** (structural breakdown).
    - **BSSI & Champion Selector (`basket_sync_engine.py`)**: Basket Structural Saturation Index $\ge 70\%$ membekukan continuation; Bilateral Composite Champion Selector memilih laggard terbaik dengan sisa runway ZCE $\ge 0.85\times\text{ATR}$ dan dorongan $\Delta\text{CSM}$.
    - **Confluence Timing (`macro_strategic_engine.py`)**: Memetakan 4 fase sirkadian (Tokyo Expansion, Tokyo Midday Lull, London Core, NY Peak Velocity) dan direktif target (`GRADE_B_C1` vs `GRADE_A_PLUS_C2`).
    - **Proteksi Siklus Hidup Posisi (`position_manager.py`)**: Midday Retracement Guard (kunci Defensive BEP jika pullback $> 65\%$ dari Peak MFE pada 11:00–13:00 WIB) dan Pre-News Emergency Shield (18:45 WIB 30 menit sebelum berita Tier-1: flatting jika $< +0.20R$, kunci BEP jika $\ge +0.20R$).
    - **Visual Sync**: Terminal CLI Bento Box Tile 3 menampilkan `Timing Phase` real-time; Dashboard SPA menyajikan metrik BSSI Saturation dan Top 1 Champion di drawer CBSS serta live badge di header bar.
    - **Verifikasi Kuantitatif**: Suite unit test `tests/test_confluence_timing_and_laggard.py` (8/8) + regression tests (23/23) **100% PASS**.
