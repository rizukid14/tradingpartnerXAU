...
Bot trading **multi-LLM consensus** (OpenAI + Gemini + DeepSeek) yang jalan di **MetaTrader 5**.
...
| `config.py` | Semua parameter + helper per-symbol (`get_timeframe`, `get_higher_timeframes`, `lot_size_for`, `default_sl/tp`, `max_spread_points`, `confidence_threshold_for`) |
| `src/core/llm_client.py` | **Build prompt dinamis per-symbol** + call 3 LLM paralel (stateless, fresh session tiap call) |
...
| `src/core/risk_engine.py` | Gate: spread, sesi, danger zone, daily loss, recovery mode, BEP tolerance |
| `src/core/mt5_connector.py` | Order send/close (retry + fill policy dinamis), history deals, market data, magic filter |
| `src/analytics/forecast_engine.py` | Forecast multi-horizon (T+15m/T+60m, invalidation, optimal entry zone) — **informational, tidak memblokir** |
...
- Git branch: `dev`. Commit terakhir sesi 8 Agustus: `bdb4c1d` (prompt dinamis).
...
- **DeepSeek konservatif**: jarang BUY (0% dalam log historis), dominan HOLD/SELL, confidence fluktuatif (0-65%). HOLD conf 0 = netral di weighted voting.
