...
| `src/analytics/basket_sync_engine.py` | **CBSS Engine & Lead-Lag Relay (9/10 Sep 2026)** — Pemetaan konstituen 8 keranjang mata uang, kalkulasi bilateral runway ZCE per-pair ($C_1/F_1$ vs $C_2/F_2$), Local Pair G3 Wall Veto (The EURAUD Law), Basket Concurrency Cap (max 2 posisi per keranjang searah / auto-route ke Paper Trade `SKIPPED_CBSS_BASKET_CAP`), **Anti-Internal Currency Hedge Gate (`check_basket_directional_conflict` — dinonaktifkan via `ENABLE_ANTI_INTERNAL_HEDGE=false` demi kebebasan peluang berbasis struktur alami & ZCE)**, **Bilateral Composite Champion Selector (`select_basket_champion`)**, dan **Lead-Lag Liquidity Relay Engine (`filter_and_rank_batch_candidates` — batch radar ranking, skip wall-exhaustion $<0.50\times\text{ATR}$, CSM vector conflict arbiter, estafet runway lapang $\ge 1.20\times\text{ATR}$)**. |
| `src/analytics/zone_confluence_engine.py` | **ZCE (True Zonal Bands & Structural Swings)** — peta zona 6-TF × multi-horizon, True Zonal Bands wick-to-body `[0.05x, 0.35x ATR_TF]`, klaster/skor J1 dengan anti-snowball chaining envelope (`<=0.75x ATR_H1`), physical boundary clearance guarantee (Floor <= cur_price <= Ceiling), Pure Sequential 4-Station Natural Ladder (C1..C4, F1..F4), Pip-Aware Zonal Consolidation $\ge 15\text{p}$, scale conflict resolution, integrasi confirmed LuxSMC structural swings (HH, LH, HL, LL) untuk akurasi benteng makro W1/D1/H4; hook `zce_walls` → MSE (`ZCE_ENABLED=true`, `ZCE_MODE=full`) |
...
| `src/analytics/macro_strategic_engine.py` | **Barrier Chamber State Machine** (6-TF Native `MN1/W1/D1/H4/H1/M30`), Density Cluster Scoring ($C_1, C_2, F_1, F_2$), Interaction Sequence Tracking (`['F1_SWEEP', 'C1_SWEEP']`), 7-State Engine, Pair-Calibrated SL Floor (35p Crosses), dan **Diurnal Confluence Timing (`evaluate_session_confluence_timing`)** (Tokyo/London/NY phase mapping & `GRADE_B_C1` vs `GRADE_A_PLUS_C2` target directives) |
