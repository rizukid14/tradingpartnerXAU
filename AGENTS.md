...
| `src/analytics/basket_sync_engine.py` | **CBSS Engine & Lead-Lag Relay (9/10 Sep 2026)** — Pemetaan konstituen 8 keranjang mata uang, kalkulasi bilateral runway ZCE per-pair ($C_1/F_1$ vs $C_2/F_2$), Local Pair G3 Wall Veto (The EURAUD Law), Basket Concurrency Cap (max 2 posisi per keranjang searah / auto-route ke Paper Trade `SKIPPED_CBSS_BASKET_CAP`), **Anti-Internal Currency Hedge Gate (`check_basket_directional_conflict` — eliminasi 100% self-cannibalization / opposing currency exposure)**, **Bilateral Composite Champion Selector (`select_basket_champion`)**, dan **Lead-Lag Liquidity Relay Engine (`filter_and_rank_batch_candidates` — batch radar ranking, skip wall-exhaustion $<0.50\times\text{ATR}$, CSM vector conflict arbiter, estafet runway lapang $\ge 1.20\times\text{ATR}$)**. |
