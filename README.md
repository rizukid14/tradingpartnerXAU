...
22. **Prompt Enhancement Blocks — "Pre-digest facts, judgment ke LLM" (13 Agustus)**: 6 blok pre-computed (zero API cost, dari df yang sama dengan candle mentah) di-inject ke prompt — **PRICE POSITION MAP** (jarak harga ke EMA/Fib/PDH/PDL/Swing dalam pts + ×ATR), **ATR REGIME** (ATR sekarang vs 5 candle lalu + %), **VOLUME CONTEXT** (volume vs 20-period avg), **PRICE NARRATIVE** (segmentasi rally/drop swing-to-swing), **SWING STRUCTURE** (semua swing highs/lows + pola HH/HL/LH/LL), **RSI DIRECTION + DIVERGENCE FACTS** (RISING/FALLING/FLAT + fakta lower-low/higher-low tanpa label). Prinsip: hitung FAKTA, sisakan judgment ke LLM — semua angka bisa diverifikasi dari candle mentah yang tetap dikirim.
23. **Adaptive Price Precision di Prompt (13 Agustus)**: level/indikator komputasi di-round ke desimal point per simbol (`_lvl_fmt`: XAU 2, EURJPY 3, GBPCHF 5 desimal) — mencegah display kontradiktif ("diff +0.00 tapi 12 pts") dan noise float EMA/ATR. Konversi diff→pts pakai `round()` bukan `int()` (float-artifact 39.9999 → 39). D1 query di-dedup: `_get_key_levels_str` return `(str, pdh, pdl, today_open)` untuk dipakai ulang Position Map/Narrative.
