...
22. **Prompt Enhancement Blocks — "Pre-digest facts, judgment ke LLM" (13 Agustus)**: 5 blok pre-computed (zero API cost, dari df yang sama dengan candle mentah) di-inject ke prompt — **PRICE POSITION MAP** (jarak harga ke EMA/Fib/PDH/PDL/Swing dalam pts + ×ATR), **ATR REGIME** (ATR sekarang vs 5 candle lalu + %), **VOLUME CONTEXT** (volume vs 20-period avg), **PRICE NARRATIVE** (segmentasi rally/drop swing-to-swing), **SWING STRUCTURE** (semua swing highs/lows + pola HH/HL/LH/LL). Prinsip: hitung FAKTA, sisakan judgment ke LLM — semua angka bisa diverifikasi dari candle mentah yang tetap dikirim.
