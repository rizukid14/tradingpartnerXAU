...
3. **SL/TP mode (default `LLM` sejak 13 Agustus)**: **mode LLM** — SL/TP murni struktur LLM (`invalidation_price`/`target_price`), dibatasi Safety Floor `max(2x spread, 0.5x default_sl_points)` untuk menghindari stops super ketat dan bloated lots. **mode ATR-Based** — SL ≥ max(2× spread, SL_MULT× ATR), TP ≥ max(2× spread, TP_MULT× ATR) (**R:R 2:1**, multiplier per AI mode: single 1.25×/2.5×, dual 1.5×/3.0×, triple 1.75×/3.5×). Ganti mode via `.env` `TP_SL_RULES`, menu setup, atau `--tpsl-rules {ATR-Based|LLM|atr|llm}`.
