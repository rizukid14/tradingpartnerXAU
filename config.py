...
# --- TOKOCRYPTO SPOT EXCHANGE ---
TOKOCRYPTO_ENABLED = _getenv_bool("TOKOCRYPTO_ENABLED", False)
TOKOCRYPTO_API_KEY = os.getenv("TOKOCRYPTO_API_KEY", "")
TOKOCRYPTO_SECRET_KEY = os.getenv("TOKOCRYPTO_SECRET_KEY", "")
TOKOCRYPTO_BASE_URL = os.getenv("TOKOCRYPTO_BASE_URL", "https://www.tokocrypto.com")
...
BREAK_EVEN_TRIGGER_POINTS = _getenv_int("BREAK_EVEN_TRIGGER_POINTS", 600)
BREAK_EVEN_PADDING_POINTS = _getenv_int("BREAK_EVEN_PADDING_POINTS", 10)

BREAK_EVEN_TRIGGER_POINTS_XAU = _getenv_int("BREAK_EVEN_TRIGGER_POINTS_XAU", 600)
...
MAX_SPREAD_POINTS_FX = _getenv_int("MAX_SPREAD_POINTS_FX", 150)
...
def reload_config():
    """Reloads all runtime trading parameters from .env into memory so separate processes stay in sync."""
    env_path = os.path.join(BASE_DIR, ".env")
    if not os.path.exists(env_path):
        return
    load_dotenv(env_path, override=True)

    global DRY_RUN, TRADING_PAUSED
    global MAX_DAILY_LOSS_USD, MAX_CONSECUTIVE_LOSSES, PAUSE_AFTER_LOSSES_MINUTES
    global MAX_OPEN_POSITIONS, BREAK_EVEN_TOLERANCE_USD, MAX_OPEN_POSITIONS_RECOVERY
    global RISK_PERCENT_XAU, RISK_PERCENT_BTC, LOT_SIZE, LOT_SIZE_XAU, LOT_SIZE_BTC, DEVIATION
    global CONFIDENCE_CONSENSUS_THRESHOLD_XAU, CONFIDENCE_CONSENSUS_THRESHOLD_BTC
    global CONSENSUS_THRESHOLD, MIN_CONSENSUS_MODELS
    global CLAUDE_MODEL, GEMINI_MODEL, OPENAI_MODEL, FORECAST_MODEL
    global CLAUDE_FALLBACK_MODEL, GEMINI_FALLBACK_MODEL, OPENAI_FALLBACK_MODEL, FORECAST_FALLBACK_MODEL
    global TP_SL_RULES, DEFAULT_SL_POINTS, DEFAULT_TP_POINTS, SL_ATR_MULTIPLIER, TP_ATR_MULTIPLIER
    global DEFAULT_SL_POINTS_XAU, DEFAULT_TP_POINTS_XAU, DEFAULT_SL_POINTS_BTC, DEFAULT_TP_POINTS_BTC
    global MAX_SPREAD_POINTS, MAX_SPREAD_POINTS_XAU, MAX_SPREAD_POINTS_FX, MAX_SPREAD_POINTS_BTC
    global TRAILING_STOP_ENABLED, TRAILING_ACTIVATION_POINTS, TRAILING_DISTANCE_POINTS
    global TRAILING_ACTIVATION_POINTS_XAU, TRAILING_DISTANCE_POINTS_XAU, TRAILING_ACTIVATION_POINTS_BTC, TRAILING_DISTANCE_POINTS_BTC
    global TRAILING_ACTIVATION_ATR_MULT_XAU, TRAILING_DISTANCE_ATR_MULT_XAU, TRAILING_ACTIVATION_ATR_MULT_BTC, TRAILING_DISTANCE_ATR_MULT_BTC
    global TRAILING_ACTIVATION_MAX_POINTS_XAU, TRAILING_ACTIVATION_MAX_POINTS_BTC
    global BREAK_EVEN_ENABLED, BREAK_EVEN_TRIGGER_POINTS, BREAK_EVEN_PADDING_POINTS
    global BREAK_EVEN_TRIGGER_POINTS_XAU, BREAK_EVEN_PADDING_POINTS_XAU, BREAK_EVEN_TRIGGER_POINTS_BTC, BREAK_EVEN_PADDING_POINTS_BTC
    global PARTIAL_CLOSE_ENABLED, PARTIAL_CLOSE_PERCENT, PARTIAL_CLOSE_TP1_POINTS
    global PARTIAL_CLOSE_TP1_POINTS_XAU, PARTIAL_CLOSE_TP1_POINTS_BTC
    global RECOVERY_MODE_ENABLED, RECOVERY_LOT_MULTIPLIER, RECOVERY_EXIT_PROFIT_USD, TRADE_COOLDOWN_SECONDS
    global DYNAMIC_CONFIG_ENABLED, FORCE_ACTIVE_ENTRY, QUANT_ANALYSIS_ENABLED, MONTE_CARLO_ENABLED
    ...
    global WEEKDAY_SYMBOL, WEEKEND_SYMBOL, TRADING_MODE
    ...
    global TOKOCRYPTO_ENABLED, TOKOCRYPTO_API_KEY, TOKOCRYPTO_SECRET_KEY, TOKOCRYPTO_BASE_URL
    ...
    MAX_CONSECUTIVE_LOSSES = _getenv_int("MAX_CONSECUTIVE_LOSSES", MAX_CONSECUTIVE_LOSSES)
    PAUSE_AFTER_LOSSES_MINUTES = _getenv_int("PAUSE_AFTER_LOSSES_MINUTES", PAUSE_AFTER_LOSSES_MINUTES)
    MAX_OPEN_POSITIONS = _getenv_int("MAX_OPEN_POSITIONS", MAX_OPEN_POSITIONS)
    BREAK_EVEN_TOLERANCE_USD = _getenv_float("BREAK_EVEN_TOLERANCE_USD", BREAK_EVEN_TOLERANCE_USD)
    MAX_OPEN_POSITIONS_RECOVERY = _getenv_int("MAX_OPEN_POSITIONS_RECOVERY", MAX_OPEN_POSITIONS_RECOVERY)
    ...
    LOT_SIZE = _getenv_float("LOT_SIZE", LOT_SIZE)
    LOT_SIZE_XAU = _getenv_float("LOT_SIZE_XAU", LOT_SIZE_XAU)
    LOT_SIZE_BTC = _getenv_float("LOT_SIZE_BTC", LOT_SIZE_BTC)
    DEVIATION = _getenv_int("DEVIATION", DEVIATION)
    ...
    CONSENSUS_THRESHOLD = _getenv_int("CONSENSUS_THRESHOLD", CONSENSUS_THRESHOLD)
    MIN_CONSENSUS_MODELS = _getenv_int("MIN_CONSENSUS_MODELS", MIN_CONSENSUS_MODELS)
    ...
    FORECAST_MODEL = os.getenv("FORECAST_MODEL", FORECAST_MODEL)
    CLAUDE_FALLBACK_MODEL = os.getenv("CLAUDE_FALLBACK_MODEL", CLAUDE_FALLBACK_MODEL)
    ...
    FORECAST_FALLBACK_MODEL = os.getenv("FORECAST_FALLBACK_MODEL", FORECAST_FALLBACK_MODEL)

    # SL/TP rules
    TP_SL_RULES = os.getenv("TP_SL_RULES", TP_SL_RULES)
    DEFAULT_SL_POINTS = _getenv_int("DEFAULT_SL_POINTS", DEFAULT_SL_POINTS)
    DEFAULT_TP_POINTS = _getenv_int("DEFAULT_TP_POINTS", DEFAULT_TP_POINTS)
    SL_ATR_MULTIPLIER = _getenv_float("SL_ATR_MULTIPLIER", SL_ATR_MULTIPLIER)
    TP_ATR_MULTIPLIER = _getenv_float("TP_ATR_MULTIPLIER", TP_ATR_MULTIPLIER)
    DEFAULT_SL_POINTS_XAU = _getenv_int("DEFAULT_SL_POINTS_XAU", DEFAULT_SL_POINTS_XAU)
    DEFAULT_TP_POINTS_XAU = _getenv_int("DEFAULT_TP_POINTS_XAU", DEFAULT_TP_POINTS_XAU)
    DEFAULT_SL_POINTS_BTC = _getenv_int("DEFAULT_SL_POINTS_BTC", DEFAULT_SL_POINTS_BTC)
    DEFAULT_TP_POINTS_BTC = _getenv_int("DEFAULT_TP_POINTS_BTC", DEFAULT_TP_POINTS_BTC)

    # Spread filter
    MAX_SPREAD_POINTS = _getenv_int("MAX_SPREAD_POINTS", MAX_SPREAD_POINTS)
    MAX_SPREAD_POINTS_XAU = _getenv_int("MAX_SPREAD_POINTS_XAU", MAX_SPREAD_POINTS_XAU)
    MAX_SPREAD_POINTS_FX = _getenv_int("MAX_SPREAD_POINTS_FX", MAX_SPREAD_POINTS_FX)
    MAX_SPREAD_POINTS_BTC = _getenv_int("MAX_SPREAD_POINTS_BTC", MAX_SPREAD_POINTS_BTC)

    # Trailing Stop
    TRAILING_STOP_ENABLED = _getenv_bool("TRAILING_STOP_ENABLED", TRAILING_STOP_ENABLED)
    TRAILING_ACTIVATION_POINTS = _getenv_int("TRAILING_ACTIVATION_POINTS", TRAILING_ACTIVATION_POINTS)
    TRAILING_DISTANCE_POINTS = _getenv_int("TRAILING_DISTANCE_POINTS", TRAILING_DISTANCE_POINTS)
    TRAILING_ACTIVATION_POINTS_XAU = _getenv_int("TRAILING_ACTIVATION_POINTS_XAU", TRAILING_ACTIVATION_POINTS_XAU)
    TRAILING_DISTANCE_POINTS_XAU = _getenv_int("TRAILING_DISTANCE_POINTS_XAU", TRAILING_DISTANCE_POINTS_XAU)
    TRAILING_ACTIVATION_POINTS_BTC = _getenv_int("TRAILING_ACTIVATION_POINTS_BTC", TRAILING_ACTIVATION_POINTS_BTC)
    TRAILING_DISTANCE_POINTS_BTC = _getenv_int("TRAILING_DISTANCE_POINTS_BTC", TRAILING_DISTANCE_POINTS_BTC)
    TRAILING_ACTIVATION_ATR_MULT_XAU = _getenv_float("TRAILING_ACTIVATION_ATR_MULT_XAU", TRAILING_ACTIVATION_ATR_MULT_XAU)
    TRAILING_DISTANCE_ATR_MULT_XAU = _getenv_float("TRAILING_DISTANCE_ATR_MULT_XAU", TRAILING_DISTANCE_ATR_MULT_XAU)
    TRAILING_ACTIVATION_ATR_MULT_BTC = _getenv_float("TRAILING_ACTIVATION_ATR_MULT_BTC", TRAILING_ACTIVATION_ATR_MULT_BTC)
    TRAILING_DISTANCE_ATR_MULT_BTC = _getenv_float("TRAILING_DISTANCE_ATR_MULT_BTC", TRAILING_DISTANCE_ATR_MULT_BTC)
    TRAILING_ACTIVATION_MAX_POINTS_XAU = _getenv_int("TRAILING_ACTIVATION_MAX_POINTS_XAU", TRAILING_ACTIVATION_MAX_POINTS_XAU)
    TRAILING_ACTIVATION_MAX_POINTS_BTC = _getenv_int("TRAILING_ACTIVATION_MAX_POINTS_BTC", TRAILING_ACTIVATION_MAX_POINTS_BTC)

    # Break-Even
    BREAK_EVEN_ENABLED = _getenv_bool("BREAK_EVEN_ENABLED", BREAK_EVEN_ENABLED)
    BREAK_EVEN_TRIGGER_POINTS = _getenv_int("BREAK_EVEN_TRIGGER_POINTS", BREAK_EVEN_TRIGGER_POINTS)
    BREAK_EVEN_PADDING_POINTS = _getenv_int("BREAK_EVEN_PADDING_POINTS", BREAK_EVEN_PADDING_POINTS)
    BREAK_EVEN_TRIGGER_POINTS_XAU = _getenv_int("BREAK_EVEN_TRIGGER_POINTS_XAU", BREAK_EVEN_TRIGGER_POINTS_XAU)
    BREAK_EVEN_PADDING_POINTS_XAU = _getenv_int("BREAK_EVEN_PADDING_POINTS_XAU", BREAK_EVEN_PADDING_POINTS_XAU)
    BREAK_EVEN_TRIGGER_POINTS_BTC = _getenv_int("BREAK_EVEN_TRIGGER_POINTS_BTC", BREAK_EVEN_TRIGGER_POINTS_BTC)
    BREAK_EVEN_PADDING_POINTS_BTC = _getenv_int("BREAK_EVEN_PADDING_POINTS_BTC", BREAK_EVEN_PADDING_POINTS_BTC)

    # Partial Close
    PARTIAL_CLOSE_ENABLED = _getenv_bool("PARTIAL_CLOSE_ENABLED", PARTIAL_CLOSE_ENABLED)
    PARTIAL_CLOSE_PERCENT = _getenv_float("PARTIAL_CLOSE_PERCENT", PARTIAL_CLOSE_PERCENT)
    PARTIAL_CLOSE_TP1_POINTS = _getenv_int("PARTIAL_CLOSE_TP1_POINTS", PARTIAL_CLOSE_TP1_POINTS)
    PARTIAL_CLOSE_TP1_POINTS_XAU = _getenv_int("PARTIAL_CLOSE_TP1_POINTS_XAU", PARTIAL_CLOSE_TP1_POINTS_XAU)
    PARTIAL_CLOSE_TP1_POINTS_BTC = _getenv_int("PARTIAL_CLOSE_TP1_POINTS_BTC", PARTIAL_CLOSE_TP1_POINTS_BTC)

    # Recovery Mode
    RECOVERY_MODE_ENABLED = _getenv_bool("RECOVERY_MODE_ENABLED", RECOVERY_MODE_ENABLED)
    RECOVERY_LOT_MULTIPLIER = _getenv_float("RECOVERY_LOT_MULTIPLIER", RECOVERY_LOT_MULTIPLIER)
    RECOVERY_EXIT_PROFIT_USD = _getenv_float("RECOVERY_EXIT_PROFIT_USD", RECOVERY_EXIT_PROFIT_USD)
    TRADE_COOLDOWN_SECONDS = _getenv_int("TRADE_COOLDOWN_SECONDS", TRADE_COOLDOWN_SECONDS)

    # Feature Toggles
    DYNAMIC_CONFIG_ENABLED = _getenv_bool("DYNAMIC_CONFIG_ENABLED", DYNAMIC_CONFIG_ENABLED)
    FORCE_ACTIVE_ENTRY = _getenv_bool("FORCE_ACTIVE_ENTRY", FORCE_ACTIVE_ENTRY)
    QUANT_ANALYSIS_ENABLED = _getenv_bool("QUANT_ANALYSIS_ENABLED", QUANT_ANALYSIS_ENABLED)
    MONTE_CARLO_ENABLED = _getenv_bool("MONTE_CARLO_ENABLED", MONTE_CARLO_ENABLED)
    FORECAST_ENABLED = _getenv_bool("FORECAST_ENABLED", FORECAST_ENABLED)
    MEMORY_CONTEXT_ENABLED = _getenv_bool("MEMORY_CONTEXT_ENABLED", MEMORY_CONTEXT_ENABLED)
    SESSION_FILTER_ENABLED = _getenv_bool("SESSION_FILTER_ENABLED", SESSION_FILTER_ENABLED)
    WEEKEND_CLOSE_ENABLED = _getenv_bool("WEEKEND_CLOSE_ENABLED", WEEKEND_CLOSE_ENABLED)
    WEEKEND_TRADING_ENABLED = _getenv_bool("WEEKEND_TRADING_ENABLED", WEEKEND_TRADING_ENABLED)
    ENABLE_BTC_ROTATION = _getenv_bool("ENABLE_BTC_ROTATION", ENABLE_BTC_ROTATION)

    # Symbol Rotation
    WEEKDAY_SYMBOL = os.getenv("WEEKDAY_SYMBOL", WEEKDAY_SYMBOL)
    WEEKEND_SYMBOL = os.getenv("WEEKEND_SYMBOL", WEEKEND_SYMBOL)
    TRADING_MODE = os.getenv("TRADING_MODE", TRADING_MODE).strip().lower()
    ...
    # Tokocrypto
    TOKOCRYPTO_ENABLED = _getenv_bool("TOKOCRYPTO_ENABLED", TOKOCRYPTO_ENABLED)
    TOKOCRYPTO_API_KEY = os.getenv("TOKOCRYPTO_API_KEY", TOKOCRYPTO_API_KEY)
    TOKOCRYPTO_SECRET_KEY = os.getenv("TOKOCRYPTO_SECRET_KEY", TOKOCRYPTO_SECRET_KEY)
    TOKOCRYPTO_BASE_URL = os.getenv("TOKOCRYPTO_BASE_URL", TOKOCRYPTO_BASE_URL)
